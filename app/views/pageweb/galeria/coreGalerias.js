var app = angular.module('MainAppGalerias', ["ui.bootstrap"]);

function mainControllerGalerias($scope, $http) {
    $scope.newGaleria = {};
    $scope.items = [];
    $scope.Regions = [];
    ...
    $scope.buscarGaleriaPorDocumento = function () {
        console.log("req.params.documento:$" + $scope.newGaleria.documento);
        $http.get('/api/galeria/' + $scope.newGaleria.documento, $scope.newGaleria)
            .success(function (data) {
                if (data.length > 0) {
                    ...
                    //console.log("$scope.newGaleria despues");
                    //console.log($scope.newGaleria);
                    $scope.sexos[0] = $scope.newGaleria.sexo;
                    $scope.tipoDocumento[0] = $scope.newGaleria.tipoDocumento;
                    $scope.estadoCivil[0] = $scope.newGaleria.estadoCivil;
                    $scope.nivelEducativo[0] = $scope.newGaleria.nivelEducativo;
                }
                //$scope.newGaleria = {}; 
    ...
    $scope.registrarGaleria = function () {
        ...
        console.log($scope.newGaleria);
        console.log("//$scope.newGaleria");
        $http.post('/api/galeria', $scope.newGaleria)
            .success(function (data) {
    ...
    // Función para editar los datos de un item
    $scope.publicarGaleria = function (newGaleria) {
        Swal({
            title: 'Esta seguro?',
            ...
            if (result.value) {
                $http.put('/api/galeria/' + newGaleria._id + '/activo/' + true)
                    .success(function (data) {
                        $scope.newGaleria = {}; // Borramos los datos del formulario
                        $scope.items = data;
                        $scope.Regions = data;
    ...
    // Función para editar los datos de un item
    $scope.activarEnPaginaPrincipal = function (newGaleria) {
        Swal({
            title: 'Esta seguro?',
            text: "Tranquilo, esta desición se puede revertir!",
            type: 'info',
            ...
            if (result.value) {
                $http.put('/api/galeriac/' + newGaleria._id + '/enpaginaprincipal/' + true)
                    .success(function (data) {
                        $scope.newGaleria = {}; // Borramos los datos del formulario
                        $scope.items = data;
                        $scope.Regions = data;
    ...
    $scope.borrarGaleria = function (newGaleria) {
        ...
        Swal({
            ...
            if (result.value) {
                ...
                $http.delete('/api/galeria/' + newGaleria._id)
                    .success(function (data) {
                        $scope.newGaleria = {};
                        $scope.items = data;
                        $scope.Regions = data;
    ...
    //$http.get('/api/galeria', { responseType: "arraybuffer" }).success(function (data) {



    $("#galerias #checkall").click(function () {
        if ($("#galerias #checkall").is(':checked')) {
            $("#galerias input[type=checkbox]").each(function () {
    ...
    $scope.GetAttributesByRegion = function (regionFrwdPageButtonClick, regionBckPageButtonClick) {
        ...
        $http.get('/api/galeria/' + $scope.RegionPage + '/' + $scope.RegionRow).success(function (data) {
            $scope.Regions = data;
    ...
    */
    ...
    $scope.selectGaleria = function (item) {
        ...
        // Obtenemos todos los datos de la base de datos para cargar la lista inicial
        /*$http.get('/api/galerias/' + $scope.RegionPage + '/' + $scope.RegionRow).succes
            $scope.items = data;
        }).error(function (data) {
            console.log('Error: ' + data);
        });*/
