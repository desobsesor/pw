var app = angular.module('MainAppNuevaNoticia', ["ui.bootstrap"]);

function mainControllerNuevaNoticia($scope, $http, $timeout) {
    $scope.newNoticia = {};
    $scope.items = {};
    ...
    $scope.newNoticia.fechaRegistro = new Date().toDateInputValue();
    ...
    $scope.cargarStoreComponentes = function () {
        $http.get('/api/categoriaNoticia').success(function (data) {
            $scope.categoriaNoticia = data;
        }).error(function (data) {
    ...
    /*$http.get('/api/noticia').success(function (data) {    // Obtenemos todos los datos de la base de datos para cargar la lista inicial
        $scope.items = data;
    }).error(function (data) {
        console.log('Error: ' + data);
    });*/
    ...
    $scope.noticia = $scope.item;
    if ($scope.noticia._id==null) {
        console.log('... >> $scope.noticia === undefined');
        $scope.selected = false;
        $scope.cargarStoreComponentes();
    } else {
        console.log("$scope.noticia");
        console.log($scope.noticia);
        $scope.newNoticia = $scope.noticia;
        $scope.selected = true;
        $scope.cargarStoreComponentes();

        $timeout(function () {
            $scope.categoriaNoticia[0] = $scope.newNoticia.categoriaNoticia;
            $scope.estadoNoticia[0] = $scope.newNoticia.estadoNoticia;
        }, 400);
    }
    //#endregion

    // Función para buscar una noticia una noticia
    $scope.buscarNoticiaPorDocumento = function () {
        console.log("req.params.titulo:$" + $scope.newNoticia.titulo);

        if ($scope.newNoticia.titulo.length > 5)
            $http.get('/api/noticia/' + $scope.newNoticia.titulo, $scope.newNoticia)
                .success(function (data) {
                    if (data.length > 0) {
                        ...
                        //console.log("$scope.newNoticia antes");
                        //console.log($scope.newNoticia);
                        $scope.newNoticia = data[0];

                        $scope.categoriaNoticia[0] = $scope.newNoticia.categoriaNoticia;
                        $scope.estadoNoticia[0] = $scope.newNoticia.estadoNoticia;
                    }
                    //$scope.newNoticia = {}; 
                    //$scope.newNoticia = data; // Borramos los datos del formulario
                })
                .error(function (data) {
                    console.log('Error: ' + data);
                });
    ...
    $scope.registrarNoticia = function () {
        console.log("$scope.newNoticia");
        //$scope.newNoticia.imagenPrincipal = { data: document.getElementById("imgTest").innerHTML.substr(45,document.getElementById("imgTest").innerHTML.length-73)}; 
        //$scope.newNoticia.imagenPrincipal = document.getElementById("srcData").innerHTML;//.substr(45,document.getElementById("imgTest").innerHTML.length-73);
        //var base64Data = $scope.newNoticia.imagenPrincipal.replace(/^data:image\/png;base64,/,"");
        //var dataBuffer = new Buffer(base64Data, 'base64');

        //$scope.newNoticia.imagenPrincipal = dataBuffer.toString();
        //$scope.newNoticia.imagenPrincipal = document.getElementById("srcData").innerHTML;
        console.log($scope.newNoticia);
        console.log("//$scope.newNoticia");
        /*var config = {
        ...
        $http.post('/api/noticia', $scope.newNoticia)//, { headers: headers }, config
            .success(function (data) {
                ...
                $scope.newNoticia = {}; // Borramos los datos del formulario
                $scope.items = data;
                $scope.tab = 1;
            })
            .error(function (data) {
    ...
    $scope.selectNoticia = function (item) {
    ...
    $("#formUpload").on("submit", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $scope.newNoticia.imagenPrincipal = ret.nombre;
                $http.get('/files').success(function (dataO) {
                    $scope.files = dataO;
                }).error(function (err) {
                    console.log('Error: ' + err);
...
function encodeImageFileAsURL() {
    ...
    if (filesSelected.length > 0) {
        ...
        fileReader.onload = function (fileLoadedEvent) {
            ...
            //newImage.ng-model = "newNoticia.imagenPrincipal";
            document.getElementById("imgTest").innerHTML = newImage.outerHTML;
            $("#srcData").css("max-width", "350px");
            //alert("Converted Base64 version is " + document.getElementById("imgTest").innerHTML);
            //$scope.newNoticia.imagenPrincial.data = document.getElementById("imgTest").innerHTML.substr(0,15) ;
